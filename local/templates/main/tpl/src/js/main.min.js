	...
	addpoints: function addpoints() {
		...
		App.points.data.forEach(function (group) {
			...
			group['points'].forEach(function (point) {
				...
				if (point.phone.length > 0) markerPhone = '<span>Телефон: <a href="tel:' + point.phone + '">' + point.phone + '</a></span>';
				if (point.email.length > 0) markerEmail = '<span>Почта: <a href="mailto:' + point.email + '">' + point.email + '</a></span>';
				if (point.url.length > 0) markerSite = '<span>Сайт: <a target="_blank" href="' + point.url + '">' + point.url + '</a></span>';

				if (point.is_distrib > 0) markerIcon = '/local/templates/main/tpl/dist/assets/images/static/marker-1.svg';

				var placemark = new ymaps.Placemark(point.coords.map(function (x) {
					return parseFloat(x);
				}), {
					balloonContentBody: '<div class="marker__descr"> ' + '<div class="marker__descr-name">' + point.name + '</div> ' + '<div class="marker__descr-content">' + '<span class="address">' + point.address + '</span>' + markerPhone + markerEmail + markerSite + '</div>  ' + '</div>'
				}, {
...
var mCont = function mCont() {
	...
	$('body').on('click', '.m-cont__tabs-item-title', function () {
		...
		var mark = placemarks[mark_id];

		App.points.map.panTo(lat_lng).then(function () {
			if (mark.getMap()) {
				mark.balloon.open();
			}
		});
